...
namespace osu.Game.Overlays.Profile.Header
{
    public class BottomHeaderContainer : CompositeDrawable
    {
        ...
        private void updateDisplay(User user)
        {
            ...
            if (user.IsOnline)
            {
                topLinkContainer.AddText("Сейчас в сети.");
                addSpacer(topLinkContainer);
            }
            else if (user.LastVisit.HasValue)
            {
                topLinkContainer.AddText("Не видели с ");
            ...
            {
                topLinkContainer.AddText("Играет с ");
                topLinkContainer.AddText(string.Join(", ", user.PlayStyles.Select(style => style.GetDescription())), embolden);

                addSpacer(topLinkContainer);
            }

            topLinkContainer.AddText("Форум ");
            topLinkContainer.AddLink($@"{user.PostCount:#,##0} постов на форуме", $"https://osu.ppy.sh/users/{user.Id}/posts", creationParameters: embolden);
            ...
            anyInfoAdded |= tryAddInfo(FontAwesome.Brands.Skype, user.Skype, @"Скайп:" + user.Skype + @"?chat");
